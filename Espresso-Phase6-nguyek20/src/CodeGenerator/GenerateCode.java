...
class GenerateCode extends Visitor {
	...
	/*
	if (!((PrimitiveType)be.getType()).isBooleanType()) {
	    be.getLeft().visit(this);
	    dataConvert(be.getLeft().getType(), be.getType());
	} else {
	    be.getRight().visit(this);
	    dataConvert(be.getRight().getType(), be.getType());
	}
	
	boolean isIntType = ((PrimitiveType)be.getType()).isIntType();
	switch (be.getOperator().getOp()) {
	case Operator.PLUS:
	    if (isIntType)
		gen.add(new Instruction(RTC.opc_iadd));
	    else
		gen.add(new Instruction(RTC.opc_fadd));
	    break;
	case Operator.MINUS:
	    if (isIntType)
		gen.add(new Instruction(RTC.opc_isub));
	    else
		gen.add(new Instruction(RTC.opc_fsub));
	    break;
	case Operator.MULT:
	    if (isIntType)
		gen.add(new Instructoin(RTC.opc_imul));
	    else
		gen.add(new Instruction(RTC.opc_fmul));
	    break;
	case Operator.DIV:
	    if (isIntType)
		gen.add(new Instruction(RTC.opc_idiv));
	    else
		gen.add(new Instruction(RTC.opc_fdiv));
	    break;
	}

	PrimitiveType t = ((PrimitiveType)be.getLeft().getType()).ceilingType((PrimitiveType)be.getRight().getType());
	be.getLeft().visit(this);
	dataConvert(be.getLeft().getType(), t);
	be.getRight().visit(this);
	dataConvert(be.getRight().getType(), t);
	if (!t.isIntType())
	    classFile.add(new Instruction(RTC.opc_fcmpg));
	switch (be.getOperator().getOp()) {
	case Operator.LT:
	    if (t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmpge, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_ifge, Lf));
	    break;
	case Operator.GT:
	    if (t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmple, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_ifle, Lf));
	    break;
	case Operator.LE:
	    if (t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmpgt, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_ifgt, Lf));
	    break;
	case Operator.GE:
	    if (t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmplt, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_iflt, Lf));
	    break;
	case Operator.EQ:
	    if(t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmpne, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_ifne, Lf));
	    break;
	case Operator.NE:
	    if (t.isIntType())
		gen.add(new JumpInstruction(RTC.opc_if_icmpeq, Lf));
	    else
		gen.add(new JumpInstruction(RTC.opc_ifeq, Lf));
	    break;
	}
	*/
	...
	public Object visitIfStat(IfStat is) {
		...
		// Generate new Lt and Lf
		Lt = newLabel();
		Lf = newLabel();

		// Generate a new done label
		String Ldone = newLabel();
		is.getExpression().visit(this);
		gen.add(new LabelInstruction(RTC.opc_label, Lt));
		is.getThenStat().visit(this);
		if (is.getElseStat() != null)
		    gen.add(new JumpInstruction(RTC.opc_goto, Ldone));
		gen.add(new LabelInstruction(RTC.opc_label, Lf));
		if (is.getElseStat() != null) {
		    is.getElseStat().visit(this);
		    gen.add(new LabelInstruction(RTC.opc_label, Ldone));
		}

		// Re-establish previous jump labels
		Lt = oldLt;
		Lf = oldLf;
		*/
	...
	public Object visitNameExpr(NameExpr ne) {
		...
		/*
		int address = ne.getName().myDecl.getAddress();
		if(((PrimitiveType)ne.getType()).isIntType())
		    gen.add(new SimpleInstruction(RTC.op_iload, address));
		else
		    gen.add(new SimpleInstruction(RTC.op_fload, address));
		*/
